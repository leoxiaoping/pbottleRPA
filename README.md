 ...
 **为什么小瓶没有图形拖拽的所谓'设计器'，用编写js脚本来替代？** 
①图形编程的上手门槛虽然更低，但目前面对长难流程项目开发和管理往往不能满足和更高维护成品。有商业价值的自动化项目往往都是长难流程。
②js脚本可以融入完整的nodejs生态，无缝引入万亿第三方功能包，同时可以使用git等代码管理工具。
③V2023.3版本 新增加鼠标操作录制自动生成简单的脚本功能。

 **为什么小瓶采用js作为脚本语言，可以用 python、java、c# 等吗？** 
JS目前是前端的主流脚本，有大量的web经验的群众基础。同时JS的前端开放性，GPT等AI工具更容易输出准确的结果。
小瓶RPA内核和脚本层采用http通信接口，虽然官方的demo脚本层选用了js，但有经验的其他语言开发者，可以随时通过最常规的http接口接入小瓶RPA。
...
1.允许个人永久免费使用本项目，包括用于个人学习、游戏娱乐、毕业设计、教学案例、公益事业和其他非商业用途;
